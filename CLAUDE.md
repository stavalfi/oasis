...
- don't prefix shell commands with `nix develop --command` or `direnv exec .` — direnv already activates the flake dev shell in the working dir, so tools like `bun`, `oxlint`, `oxfmt`, `node`, `sops`, `age`, etc. are already on PATH. Just run them directly.
