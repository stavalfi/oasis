...
const blogSummaryRequiredSchema = z.object({
  ...
  JIRA_SUMMARY_CLOUD_ID: z.string(),
...
const blogSummaryEnvSchema = z
  .object({
    ...
    JIRA_SUMMARY_CLOUD_ID: optionalString,
  ...
  .transform((vars, ctx) => {
    ...
    if (!required.success) {
      ctx.addIssue({
        ...
        message:
          ...
          "JIRA_SUMMARY_BASE_URL, JIRA_SUMMARY_CLOUD_ID, JIRA_SUMMARY_EMAIL, " +
          "JIRA_SUMMARY_API_TOKEN, and JIRA_SUMMARY_PROJECT_KEY together, or none of them.",
      });
      return z.NEVER;
    }
    const { data } = required;
    return {
      ...
      jira: {
        ...
        cloudId: data.JIRA_SUMMARY_CLOUD_ID,
...
export const config = deepFreeze({
  ...
  constants: {
    ...
    kafka: {
      ...
      // Partitions to create the topic with if it does not exist yet. One is
      // enough for this low-volume topic (ordering is per-key anyway).
      topicPartitions: 1,
    },
    ...
    // How long to pause the blog-summary partition after an AIonLabs 429, giving
    // the rate-limit window time to clear before the message is redelivered.
    rateLimitPauseMs: 2 * 60 * 1000,
